...
class CommittorNN(nn.Module):
    def __init__(self):
        ...
        self.register_buffer('reactant_center',torch.tensor([-75,75],dtype=torch.float32))
        self.register_buffer('product_center',torch.tensor([80,-80],dtype=torch.float32))
        self.radius = 10

    def smooth_transition(self,x,center):
        d2 = torch.sum((x-center)**2, dim=1, keepdim=True)
        r_plus_sq = (self.radius + 2)**2
