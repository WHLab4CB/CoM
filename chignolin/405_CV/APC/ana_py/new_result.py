...
if i_cycle < 2:
    cell_list = [i for i in range(n_cell)]
            ...
            if os.path.exists(name_log):
                ...
                if lines:
                    if j == 0:
                        ...
                        for m in range(4,len(first_line)):
                            print(first_line[m],end=" ",file=f_traj_out)
                        print(first_line[2],end=" ",file=f_traj_out)
                        last_line = lines[-1].strip().split()
                        for m in range(4,len(last_line)):
                            print(last_line[m],end=" ",file=f_traj_out)
                        print(last_line[2],end=" ",file=f_traj_out)
                    else:
                        if j == (n_traj-1):
                            last_line = lines[-1].strip().split()
                            for m in range(4,len(last_line)):
                                print(last_line[m],end=" ",file=f_traj_out)
                            print(last_line[2],file=f_traj_out)
                            #print(last_line[4],last_line[5],last_line[6],last_line[7],last_line[2],file=f_traj_out)
                        else:
                            last_line = lines[-1].strip().split()
                            for m in range(4,len(last_line)):
                                print(last_line[m],end=" ",file=f_traj_out)
                            print(last_line[2],end=" ",file=f_traj_out)
                            #print(last_line[4],last_line[5],last_line[6],last_line[7],last_line[2],end=" ",file=f_traj_out)
