...
x = data[:,:405].astype(np.float64)
c = data[:,406].astype(np.float64)
...
class CommittorNN(nn.Module):
    def __init__(self):
        ...
        #self.radius= 0.2
        self.selected_cols = torch.tensor([74, 78, 191, 195], dtype=torch.long, device=device)

    def smooth_transition_r(self,x,center):
        #sum_x = torch.sum(x,dim=1,keepdim=True)
        sum_x = torch.sum(x[:, self.selected_cols], dim=1, keepdim=True)
        d2 = sum_x-center+0.2
